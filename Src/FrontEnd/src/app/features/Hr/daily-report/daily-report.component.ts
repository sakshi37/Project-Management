...
export class DailyReportComponent {
  ...
  todayString: string = new Date().toISOString().slice(0, 10);
   ...
   records: MissPushOutModel[] = [];
    ...
    pageSize: number = 1; // current items per page
...
sortData(column: string) {
  ...
  const getValue = (item: MissPushOutModel) => {
    switch(column) {
      ...
      case 'endDate':
      return item.endDate ? new Date(item.endDate) : new Date(0);
      default:
      return '';  // fallback
    }
  };
...
getSortIcon(column: string) {
  if (this.sortColumn !== column) return 'fa-sort'; // default sort icon
